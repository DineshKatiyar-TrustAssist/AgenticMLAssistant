...
code_execution_agent = Agent(
    ...
    The code to execute has been generated by the CoderAgent and reviewed by the ReviewerAgent.
    The reviewed code is stored in the session state with the key "reviewed_code" from the ReviewerAgent.
    If "reviewed_code" is not available, use "code" from the CoderAgent as a fallback.
    
    IMPORTANT: Follow these steps:
    1. Get the code from session state - look for "reviewed_code" first, if not found use "code"
    2. Call the extract_python_code tool with that code to extract clean Python code
    3. Then, call the execute_code tool with the extracted code to execute it
    4. Analyze the execution output and create a detailed execution summary
    5. Add the execution summary to execution_result in the following format:
